...
Router atorRouter(AtoresDatabaseHelper atorDb, FilmesDatabaseHelper filmeDb) {
  ...
  // POST /atores/lote — Criar vários atores de uma vez
  router.post('/atores/lote', (Request request) async {
    try {
      ...
      for (final data in lista) {
        ...
        if (map['nome'] == null || map['filmeId'] == null) {
          return Response(400,
            body: jsonEncode({'erro': 'Cada ator precisa de "nome" e "filmeId"'}),
            headers: {'Content-Type': 'application/json'},
          );
        }
  ...
  // DELETE /atores/<id> — 204 No Content
