...
Router filmeRouter(FilmesDatabaseHelper filmeDb) {
  ...
  // GET /filmes — Listar todos, com filtros opcionais
  ...
  // POST /filmes/lote — Criar vários filmes de uma vez
  router.post('/filmes/lote', (Request request) async {
    try {
      final body = await request.readAsString();
      final List<dynamic> lista = jsonDecode(body) as List<dynamic>;
      ...
      for (final data in lista) {
        ...
        criados.add(criado.toJson());
      }

      return Response(201,
        body: jsonEncode(criados),
        headers: {'Content-Type': 'application/json'},
      );
    } catch (e) {
      return Response(400,
  ...
  // PUT /filmes/lote — Atualizar vários filmes de uma vez
  router.put('/filmes/lote', (Request request) async {
    try {
      final body = await request.readAsString();
      final List<dynamic> lista = jsonDecode(body) as List<dynamic>;
      ...
      for (final data in lista) {
        ...
        final resultado = await filmeDb.update(id, filmeEditado);
        if (resultado != null) {
          atualizados.add(resultado.toJson());
        }
      }

      return Response.ok(
        jsonEncode(atualizados),
        headers: {'Content-Type': 'application/json'},
      );
    } catch (e) {
      return Response(400,
  ...
  // DELETE /filmes/<id> — 204 No Content
